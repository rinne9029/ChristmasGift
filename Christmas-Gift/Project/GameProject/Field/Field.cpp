...
std::list<CVector3D> Field::ms_nodes =
{
	...
	CVector3D(-1.119953,0.1,5.792867),
	CVector3D(-0.969914,0.1,16.858345),
	CVector3D(-7.946468,0.1,6.155985),
	//CVector3D(-0.200370,0.960001,3.097259),
	// 
...
void Field::CreateLights()
{
	...
	for (CVector3D lightsPos : ms_lights)
	{
		...
		{
			//2～番は部屋用ライト番号
			new Light(lightsPos, m_lightNo,0);
			m_lightNo++;
		}
		else if(m_lightNo <= 20)
		{
			//2～番は部屋用ライト番号
			new Light(lightsPos, m_lightNo,1);
			m_lightNo++;
		}
		else
		{
			////2～番は部屋用ライト番号
			new Light(lightsPos, m_lightNo, 2);
			m_lightNo++;
		}
